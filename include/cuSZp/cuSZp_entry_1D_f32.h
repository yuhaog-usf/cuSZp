...
// Generic block-size variants (blockSize must be 32, 64, 128, or 256).
void cuSZp_compress_1D_plain_f32_blkN(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, float errorBound, int blockSize, cudaStream_t stream = 0);
void cuSZp_decompress_1D_plain_f32_blkN(float* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, float errorBound, int blockSize, cudaStream_t stream = 0);
