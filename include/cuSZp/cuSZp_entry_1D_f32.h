...
// Block-size=64 variants (2 warps per block).
void cuSZp_compress_1D_plain_f32_blk64(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, float errorBound, cudaStream_t stream = 0);
void cuSZp_decompress_1D_plain_f32_blk64(float* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, float errorBound, cudaStream_t stream = 0);
