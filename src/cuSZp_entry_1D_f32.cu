...
// ============================================================================
// Variable datablock-size entry functions (_dblk)
// ============================================================================

#define LAUNCH_COMPRESS_DBLK(DBS) \
    cuSZp_compress_kernel_1D_plain_f32_dblk<DBS><<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_oriData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle)

void cuSZp_compress_1D_plain_f32_dblk(float* d_oriData, unsigned char* d_cmpBytes, size_t nbEle, size_t* cmpSize, float errorBound, int dblockSize, cudaStream_t stream)
{
    int bsize = tblock_size; // CUDA block = 32, unchanged
    int gsize = (nbEle + bsize * thread_chunk - 1) / (bsize * thread_chunk);
    int cmpOffSize = gsize + 1;
    const size_t rate_ofs =
        (size_t)gsize * bsize * thread_chunk / dblockSize;
    ...
    err = cudaMalloc((void**)&d_cmpOffset, sizeof(unsigned int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_compress_1D_plain_f32_dblk: cudaMalloc(d_cmpOffset) failed: %s\n", cudaGetErrorString(err)); if (cmpSize) *cmpSize = 0; return; }
    cudaMemsetAsync(d_cmpOffset, 0, sizeof(unsigned int)*cmpOffSize, stream);
    err = cudaMalloc((void**)&d_locOffset, sizeof(unsigned int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_compress_1D_plain_f32_dblk: cudaMalloc(d_locOffset) failed: %s\n", cudaGetErrorString(err)); if (cmpSize) *cmpSize = 0; goto cleanup_dblk_cmp; }
    cudaMemsetAsync(d_locOffset, 0, sizeof(unsigned int)*cmpOffSize, stream);
    err = cudaMalloc((void**)&d_flag, sizeof(int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_compress_1D_plain_f32_dblk: cudaMalloc(d_flag) failed: %s\n", cudaGetErrorString(err)); if (cmpSize) *cmpSize = 0; goto cleanup_dblk_cmp; }
...

    {
        ...
        dim3 gridSize(gsize);
        switch (dblockSize) {
            case 32:  LAUNCH_COMPRESS_DBLK(32);  break;
            case 64:  LAUNCH_COMPRESS_DBLK(64);  break;
            case 128: LAUNCH_COMPRESS_DBLK(128); break;
            case 256: LAUNCH_COMPRESS_DBLK(256); break;
            default:
                fprintf(stderr, "cuSZp_compress_1D_plain_f32_dblk: unsupported dblockSize=%d\n", dblockSize);
                if (cmpSize) *cmpSize = 0;
                goto cleanup_dblk_cmp;
        }
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            fprintf(stderr, "cuSZp_compress_1D_plain_f32_dblk: kernel launch failed: %s\n", cudaGetErrorString(err));
            if (cmpSize) *cmpSize = 0;
            goto cleanup_dblk_cmp;
        }
    }
...
#define LAUNCH_DECOMPRESS_DBLK(DBS) \
    cuSZp_decompress_kernel_1D_plain_f32_dblk<DBS><<<gridSize, blockSize, sizeof(unsigned int)*2, stream>>>(d_decData, d_cmpBytes, d_cmpOffset, d_locOffset, d_flag, errorBound, nbEle)

void cuSZp_decompress_1D_plain_f32_dblk(float* d_decData, unsigned char* d_cmpBytes, size_t nbEle, size_t cmpSize, float errorBound, int dblockSize, cudaStream_t stream)
{
    int bsize = tblock_size;
    int gsize = (nbEle + bsize * thread_chunk - 1) / (bsize * thread_chunk);
    int cmpOffSize = gsize + 1;
    ...
    err = cudaMalloc((void**)&d_cmpOffset, sizeof(unsigned int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_decompress_1D_plain_f32_dblk: cudaMalloc(d_cmpOffset) failed: %s\n", cudaGetErrorString(err)); return; }
    cudaMemsetAsync(d_cmpOffset, 0, sizeof(unsigned int)*cmpOffSize, stream);
    err = cudaMalloc((void**)&d_locOffset, sizeof(unsigned int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_decompress_1D_plain_f32_dblk: cudaMalloc(d_locOffset) failed: %s\n", cudaGetErrorString(err)); goto cleanup_dblk_dec; }
    cudaMemsetAsync(d_locOffset, 0, sizeof(unsigned int)*cmpOffSize, stream);
    err = cudaMalloc((void**)&d_flag, sizeof(int)*cmpOffSize);
    if (err != cudaSuccess) { fprintf(stderr, "cuSZp_decompress_1D_plain_f32_dblk: cudaMalloc(d_flag) failed: %s\n", cudaGetErrorString(err)); goto cleanup_dblk_dec; }
    cudaMemsetAsync(d_flag, 0, sizeof(int)*cmpOffSize, stream);

    {
        ...
        dim3 gridSize(gsize);
        switch (dblockSize) {
            case 32:  LAUNCH_DECOMPRESS_DBLK(32);  break;
            case 64:  LAUNCH_DECOMPRESS_DBLK(64);  break;
            case 128: LAUNCH_DECOMPRESS_DBLK(128); break;
            case 256: LAUNCH_DECOMPRESS_DBLK(256); break;
            default:
                fprintf(stderr, "cuSZp_decompress_1D_plain_f32_dblk: unsupported dblockSize=%d\n", dblockSize);
                goto cleanup_dblk_dec;
        }
        err = cudaGetLastError();
        if (err != cudaSuccess) {
            fprintf(stderr, "cuSZp_decompress_1D_plain_f32_dblk: kernel launch failed: %s\n", cudaGetErrorString(err));
            goto cleanup_dblk_dec;
        }
    }
