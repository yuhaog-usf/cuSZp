                                                ...
                                                const float eb, const size_t nbEle)
{
    ...
    if(warp>0)
    {
        if(!lane)
        {
            ...
            excl_sum[warp_in_block] = loc_excl_sum;
        }
        __syncwarp();
    }
    ...
    if(!lane) base_idx[warp_in_block] = excl_sum[warp_in_block] + rate_ofs;
    __syncwarp();

    unsigned int base_cmp_byte_ofs = base_idx[warp_in_block];
    unsigned int cmp_byte_ofs;
    unsigned int tmp_byte_ofs = 0;
    unsigned int cur_byte_ofs = 0;
    for(int j=0; j<block_num; j++)
                                                  ...
                                                  const float eb, const size_t nbEle)
{
    ...
    if(warp>0)
    {
        if(!lane) cmpOffset[warp] = excl_sum[warp_in_block];
        __threadfence();
        if(!lane) flag[warp] = 2;
        __threadfence();
    }
