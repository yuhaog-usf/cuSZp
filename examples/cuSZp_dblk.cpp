/*
 * cuSZp_dblk.cpp — cuSZp variable datablock-size GPU benchmark
 *
 * Benchmarks cuSZp plain-1D-f32 with datablock size = {32, 64, 128, 256}.
 *
 * Usage:
 *   ./cuSZp_dblk -i <datafile> -eb <rel|abs> <bound> [-w <warmup>] [-r <repeat>]
 *
...
int main(int argc, char **argv)
{
    ...
    printf("%-8s  %10s  %12s  %12s  %12s  %12s  %12s  %12s  %s\n",
           "DBlk", "CR", "Comp(GB/s)", "Comp_P95", "Decomp(GB/s)", "Dec_P95", "MaxAbsErr", "PSNR(dB)", "Check");
    printf("--------  ----------  ------------  ------------  ------------  ------------  ------------  ------------  -----\n");

    printf("\n# CSV_START\n");
    printf("dataset,elements,valueRange,errMode,userBound,absErr,warmup,repeat,"
           "dblockSize,CR,cmpBytes,"
    ...
    for (int bi = 0; bi < num_dblock_sizes; bi++) {
        ...
        for (int w = 0; w < warmup; w++) {
            ...
            cuSZp_compress_1D_plain_f32_dblk(d_oriData, d_cmpBytes, nbEle, &cmpSize, absErr, dbs, stream);
            cuSZp_decompress_1D_plain_f32_dblk(d_decData, d_cmpBytes, nbEle, cmpSize, absErr, dbs, stream);
        ...
        printf("%-8d  %10.2f  %12.2f  %12.2f  %12.2f  %12.2f  %12.2e  %12.1f  %s\n",
               dbs, cr, comp_gbs, comp_p95_gbs, dec_gbs, dec_p95_gbs,
