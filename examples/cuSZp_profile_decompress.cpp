/*
 * cuSZp_profile_decompress.cpp — Nsight Systems profile-only target for decompress.
 *
 * Flow:  load data -> H2D -> 3 warmup iterations -> 1 unprofiled compress
 *        (to produce valid cmpBytes/cmpSize) -> cudaProfilerStart()
 *        -> 1 decompress call + stream sync -> cudaProfilerStop() -> cleanup.
 *
 * Pair with: nsys profile --capture-range=cudaProfilerApi --capture-range-end=stop ...
 *
 * Usage:
 *   ./cuSZp_profile_decompress -i <datafile> -eb <rel|abs> <bound>
 */
...
int main(int argc, char **argv)
{
    ...
    // Produce valid compressed payload for the profiled decompress.
    cudaMemsetAsync(d_cmpBytes, 0, rawBytes, stream);
    cuSZp_compress_1D_plain_f32(d_oriData, d_cmpBytes, nbEle, &cmpSize, absErr, stream);
    cudaStreamSynchronize(stream);

    // ===== Profiled region: exactly one decompress call =====
    cudaProfilerStart();
    cuSZp_decompress_1D_plain_f32(d_decData, d_cmpBytes, nbEle, cmpSize, absErr, stream);
    cudaStreamSynchronize(stream);
    cudaProfilerStop();
